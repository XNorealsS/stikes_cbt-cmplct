...
<div class="space-y-6">
    ...
    <div class="mb-4">
        <p class="text-xs text-slate-400 mb-1">STIKESMU &gt; Dosen &gt; Analisis Butir Soal</p>
        <div class="flex flex-wrap items-start justify-between gap-3 pb-3 border-b-2 border-green-700">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-0.5">Evaluasi kualitas soal (tingkat kesukaran, daya beda, reliabilitas) untuk ujian yang Anda ampu.</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white p-4 rounded-xl border border-gray-250 shadow-sm">
        <form method="GET" action="{{ route('dosen.analisis.index') }}" class="flex flex-col sm:flex-row items-end gap-4">
            <div class="flex-grow w-full">
                <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Pilih Sesi Ujian Anda</label>
            ...
            <div class="flex space-x-2 w-full sm:w-auto">
                ...
                <a href="{{ route('dosen.analisis.export', $selectedExam->id) }}" class="bg-secondary hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg text-xs uppercase tracking-wider transition shadow-sm text-center w-full sm:w-auto cursor-pointer flex items-center justify-center space-x-1.5">
...
<script>
    function runAnalysis(examId) {
        Swal.fire({
            ...
            preConfirm: () => {
                return axios.post(`/dosen/analisis-soal/${examId}/run`)
                    .then(res => {
